...
const Updater = () => {
    ...
    const [checkinStatus, setCheckingStatus] = useState(false)
    ...
    const needUpdate =  async() => {
        ...
        var myRequest = new Request("updates.json")


        const responce = await fetch(myRequest, myInit)
    ...
    const checkForUpdates = () => {
        if (!checkinStatus) {
            ...
            needUpdate().then(updateAvailable => {
                if (updateAvailable) {
                    navigator.serviceWorker.ready.then(registration => {
                        registration.active.postMessage({ type: "do-soft-update" })
                    })
                } else {
    ...
    return (
        <div className="Updater">
            <button className={"check-btn" + (checkinStatus ? " active" : "")} onClick={checkForUpdates}>
            ...
            <div className="descr">
                <span className="title">{ activeUpdate ? "Доступно обновление" : "Обновления" }</span>
                <span className="info">{ checkinStatus ? "Проверка.." : "Последняя проверка: 18:30" }</span>
            </div>
            { activeUpdate && <button className="update-btn" onClick={() => location.reload()}>Обновить</button> }
            { !activeUpdate && <span className="version">V1.0.3</span> }
