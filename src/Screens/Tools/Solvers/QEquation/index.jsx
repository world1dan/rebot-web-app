        ...
        <div className="QEquation">
            ...
            { results && <div className="result">
                <div className="solution-info">
                    <div className="line">D = <span className="secondary">{ b }² - 4 * { a !== "" ? a : 1 } * { c }</span> = { results[2] }</div>
                    <div className="line">x₁ = <span className="secondary">-{ b } + {Math.sqrt(results[2])} / 2 * { a !== "" ? a : 1 }</span> = { parseFraction(results[0]) }</div>
                    <div className="line">x₂ = <span className="secondary"> -{ b } - {Math.sqrt(results[2])} / 2 * { a !== "" ? a : 1 }</span> = { parseFraction(results[1]) }</div>
                </div>
            </div> }
        </div>
    )
