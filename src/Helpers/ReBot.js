    ...
    constructor(manifest) {
        ...
        document.querySelectorAll("#rebot header button").forEach((el) => {
            if (el.id != "brainly" && el.id != "math") {
                el.onclick = () => { getInput((id, nums) => { this.open(id, nums) }, el.id) }
            }
        })
    ...
    open(id, num) { 
        ...
        for (let num of nums) {
            ...
            if (!subj.section) {
                ...
                box.querySelector("#view").src = subj.url.replace("*", num - (subj.title == "Химия" ? 1 : 0)).replace("?", 1)
                ...
                while (i < 12) {
                    ...
                    img.src = subj.url.replace("*", num - (subj.title == "Химия" ? 1 : 0)).replace("?", i)
